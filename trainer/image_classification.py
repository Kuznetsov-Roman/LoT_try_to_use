...
def evaluate(teacher, student, loader, epoch):
    ...
    '| Teacher Test F1: %.3f | Student Test F1: %.3f | Teacher Top 5 Accuracy: %.3f | Student Top 5 Accuracy: %.3f | Time: %.3f | '
            % (step, avg_teacher_loss, 100. * teacher_correct / total, avg_student_loss, 100. * student_correct / total, end-start, 
               f1_score(targets.cpu().numpy(), teacher_pred.max(1)[1].cpu().numpy(), average="macro"), f1_score(targets.cpu().numpy(), student_pred.max(1)[1].cpu().numpy(), average="macro"), 
               top_k_accuracy_score(targets.cpu().numpy(), teacher_pred.cpu().numpy(), k=5, labels = np.arange(100)), top_k_accuracy_score(targets.cpu().numpy(), student_pred.cpu().numpy(), k=5, labels = np.arange(100))))
